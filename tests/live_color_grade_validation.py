#!/usr/bin/env python3
"""Live validation for the Color / Grade kernel.

Creates a disposable Resolve project, generates synthetic color media, probes
grade, node graph, LUT, DRX, gallery, and color-group surfaces, then deletes the
...
def _install_mcp_stubs() -> None:
    ...
    def stdio_server(*args, **kwargs):
        raise RuntimeError("stdio_server is not used by the live Color / Grade harness")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Live Color / Grade kernel validation harness")
    parser.add_argument("--keep-open", action="store_true", help="Leave the disposable project open for inspection.")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=None,
        help="Directory for Color / Grade JSON/Markdown reports. Defaults to a temp directory.",
    )
