...
| `plan_cut` | plan | **Relay `plan["grid_available"]`** — it selects between two structurally different montages (#193). True: a beat-locked cut with an arrangement, so `motion={}` gives the full pass and the `retime`/`flash`/`shake`/`fadeout` flags exist. False (tempo confidence below `MIN_TEMPO_CONFIDENCE`): an onset-density cut with no arrangement, so segments carry no `motion` and no flags, `motion={}` applies only vignette/grain/letterbox, and `polish_timeline` has nothing to author. Also relay `plan["look_bucket_basis"]` (`scout`/`ffmpeg_signature`/`mixed`/`default`) — on `default` the match CDL is an identity and `finish` still reports `applied: N/N`, because that counts successful SetCDL calls, not shots that changed. Craft levers (#193 phase 6.2): shot SIZE (`visual.shot_size`, previously captured and never read) is a tiebreak in shot selection so the cut alternates wide/medium/close; the zoom move varies per shot in direction and magnitude (`ZOOM_VARIATION_CYCLE`, keyed on the shot ordinal — `beat_index` is always even and would only ever hit half the cycle); the match CDL corrects contrast and saturation as well as exposure and white balance, with every axis clamped, the white-balance tilt proportional to the MEASURED cast rather than a fixed ±0.05 on a 3-way label, and a single-bucket match reduced to an identity; flat/log footage is DETECTED (`plan["flat_footage_clips"]`) and never converted — a CDL cannot do log→Rec709. Scout-offer round trip: the offer carries its token under BOTH `confirm_token` (deep_vision's name) and `scout_confirm_token`, and this action accepts either — echoing the key you were handed used to re-serve the identical offer forever with no error (#193). Builds the CutList: word-level Pass-1 (fillers/false starts; cue-level fallback), dead-air windows, duration fit, jump-cut smoothing (b-roll via similarity, else punch-in), title, music gain via loudness. Returns the markdown checkpoint summary. **Montage**: `scout` (default true) may return a `deep_vision` in-point offer instead of a plan — see the genre section; `scout_confirm_token` completes its handshake. |
...
| `build_timeline` | execute | **Montage segments carry no production audio** (#193 phase 5.1): they never set `audio_track_indices`, and the `or [1]` fallback used to mirror every B-roll shot's camera audio onto A1 at unity gain under the music. Opt back in per segment by setting `audio_track_indices` explicitly. Pins `timelineFrameRate` but NOT the raster — set `timelineResolutionWidth`/`Height` via `project_settings` before `start_brief` or a 4K/vertical montage silently delivers the project's raster. Result key paths: only `usage_summary` is inside `readback`; `build_errors`, `title` and `beat_alignment` are siblings at the top level, and `punch_ins` is talking-head only (always `[]` on montage). `beat_alignment {checked, deviations}` is montage-only and present only on a grid-locked plan — it proves Resolve placed items where the PLAN said, not that the plan is on the music (`grid_available` decides that). Append-rebuild: intro title at the head of V1, V1 speech with `mediaType:2` audio mirroring, V2 b-roll positioned appends, punch-in `ZoomX`/`ZoomY`, A2 music trimmed to the cut (ducked bed only when consented). Readback-verified; persists the intro-title `record_offset` for the polish pass. |
...
| `finish` | execute | Asserts `ExportAudio: True` for montage plans — that setting is project-wide sticky state and `render(build_proxies)` writes it `False` without restoring it, so a proxy build earlier in the session left the next montage render silent with every check green (#193 phase 5.4). No loudness normalisation exists anywhere on the montage path: `approve_cut` forces the static bed and `plan_cut` skips the loudness pass, so measure with `media_analysis(analyze_file)` → `loudness.metrics.integrated_lufs` afterwards. Grade (`lut_path` / `cdl` / `drx_path`), optional subtitles (`CreateSubtitlesFromAudio`), validated render (`prepare_render_job` → `StartRendering`); verifies the output file exists and reports its path. **Montage**: `grade={"match": …}` applies per-look-bucket CDLs as stage 1 beneath the uniform look — both the explicit `{bucket: {"cdl": …}}` and the plan's own `look_buckets` (`{bucket: <raw CDL>}`, from `montage_edit.compute_match_cdls`) are accepted, so the suggestion can be fed straight back. **Only `lut_path` composes with `match`** (`SetLUT` is a separate node control); a uniform `cdl` is `item.SetCDL` on the same items and REPLACES it, and `drx_path` replaces the whole graph — `graded["match"]["overwritten_by"]` + `warning` report the destructive combination, because `match.applied` counts what was set, not what survived; `motion={}` applies the beat-locked Fusion motion/flash/shake/fadeout/look pass — the beat pulse itself is opt-in per section (`MOTION_PULSE_AMP`: `drop`/`high` only, #209), so most segments carry `amp: 0` and get only the ramp; `motion={"look": false}` drops vignette/grain/letterbox, `motion={"pulse": false}` forces the pulse off everywhere the plan stored one; `qc` runs after a successful render unless `qc=false`. `target` selects the `built` (default) or `polished` timeline. |
