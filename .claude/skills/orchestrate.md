...
description: Resumable ingest-to-deliver post-production conductor in the DaVinci Resolve MCP. Apply when a task spans multiple domains (edit AND grade AND deliver, "take this footage all the way to delivery") AND needs to survive a context reset across sessions. For a single-domain task, or a talking-head edit that fits in one session, use the matching domain skill (resolve-auto-edit, resolve-color, ...) directly instead.
---
