...
| Resolve connection bootstrap | `resolve_connection.py`, `resolve_launch.py`, `resolve_autolaunch.py` | `granular/` | connector core; both live servers depend on it. `resolve_launch.py` is the ONE spawn/poll path — `live_connection.py` and `granular/common.py` both delegate; never reimplement Popen in either (#104). Every `scriptapp()` call site must call `locale_guard.restore()` right after — the bridge resets the C locale to POSIX **mid-process**, making every unencoded `open()`/`subprocess(text=True)` ASCII-only (#121). Mid-process is what makes it reachable: a C/POSIX *startup* locale turns CPython's UTF-8 mode on instead (#127). Text IO everywhere in `src/` names `encoding="utf-8"` so the locale is never load-bearing; `tests/test_text_encoding_guard.py` fails on a new unencoded site (#124). `resolve_autolaunch.py` is a dependency-free thread-local (`passive_resolve_probe()`) both bootstraps consult: MCP **resource** handlers run inside it so a passive host poll never triggers a ~60s launch (#143) |
