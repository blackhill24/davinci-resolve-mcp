...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f"mcp_benchmark_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"),
...
def send_request(tool_name: str, params: Dict[str, Any]) -> Tuple[Dict[str, Any], float]:
...
def measure_system_resources() -> Dict[str, float]:
    ...
    try:
        ...
        for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
            if proc.info['cmdline'] and any('resolve_mcp_server.py' in cmd for cmd in proc.info['cmdline'] if cmd):
                # Get memory usage
...
def benchmark_operation(operation_name: str, tool_name: str, params: Dict[str, Any], 
                      iterations: int = 10, delay: float = 0.5) -> Dict[str, Any]:
    ...
    logger.info(f"  Min/Max: {stats['min_time'] * 1000:.2f}ms / {stats['max_time'] * 1000:.2f}ms")
...
def print_summary(results: Dict[str, Any]) -> None:
    ...
    for key, value in results.items():
        if key not in ["initial_resources", "final_resources", "resource_change"] and isinstance(value, dict):
    ...
    # Overall stats
    if response_times:
        logger.info(f"Overall average response time: {statistics.mean(response_times) * 1000:.2f}ms")
    if success_rates:
        logger.info(f"Overall success rate: {statistics.mean(success_rates) * 100:.1f}%")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Benchmark the DaVinci Resolve MCP Server")
