...
| Resolve connection bootstrap | `resolve_connection.py`, `resolve_launch.py` | `granular/` | connector core; both live servers depend on it. `resolve_launch.py` is the ONE spawn/poll path — `live_connection.py` and `granular/common.py` both delegate; never reimplement Popen in either (#104). Every `scriptapp()` call site must call `locale_guard.restore()` right after — the bridge resets the C locale to POSIX, making every unencoded `open()`/`subprocess(text=True)` ASCII-only (#121) |
