...
| `grade` | `timeline_item_color.safe_apply_drx` / `safe_set_cdl` | No-op done unless the brief (or the call) supplies `options.grade` — G2's vision checkpoint still fires regardless. |
